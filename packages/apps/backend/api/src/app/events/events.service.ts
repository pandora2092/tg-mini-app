...
export class EventsService {
  ...
  async getAllEventsByCity(city: string): Promise<EventDto> {
    const { data } = await firstValueFrom(
      this.httpService.get<EventDto>(
        `${apiExternalUrl}/public-api/v1.4/events/?lang=&rufields=title,price,images,description,dates,categories,tagline,tags&location=${city}&actual_since=${actual_since}`,
      ),
    );
    return data;
  ...
  async getAllEventsByCategories(
    ...
    const { data } = await firstValueFrom(
      this.httpService.get<EventDto>(
        `${apiExternalUrl}/public-api/v1.4/events/?lang=ru&fields=title,price,images,description,dates,categories,tagline,tags,dates,body_text,place,id&location=${city}&actual_since=${actualSince}&&actual_until=${actualUntil}&categories=${categories}&page=${page}&page_size=${pageSize}`,
      ),
    );
    return data;
  }
  ...
  async getAllEventCategories(): Promise<EventCategoriesDto> {
    const { data } = await firstValueFrom(
      this.httpService.get<EventCategoriesDto>(
        `${apiExternalUrl}/public-api/v1.4/event-categories/?lang=ru`,
      ),
    );
    return data;
  }
